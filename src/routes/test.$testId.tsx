...
export const Route = createFileRoute("/test/$testId")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Timed CEE mock test with question palette, review marks and auto-submit." },
...
function TestRunner() {
  ...
  const submit = useCallback(
    async (auto = false) => {
      ...
      void navigate({ to: "/result/$attemptId", params: { attemptId: data as string }, replace: true });
    },
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-4 py-8 lg:grid-cols-[1fr_260px]">
        <Card className="card-elevated">
          <CardContent className="p-6">
            ...
            <p className="mt-4 text-lg font-medium leading-relaxed">{current?.question}</p>
              ...
              {LETTERS.map((letter) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex w-full items-start gap-3 rounded-lg border border-border bg-card p-4 text-left transition-colors hover:border-primary/50 hover:bg-muted/60",
                      selected && "border-primary bg-primary/5 ring-1 ring-primary",
                    )}
                  >
                    <span
                      className={cn(
                        "grid size-7 shrink-0 place-items-center rounded-full border border-border text-sm font-semibold",
        ...
        <aside className="card-elevated h-fit p-4">
          ...
          <div className="mt-3 grid grid-cols-6 gap-1.5 lg:grid-cols-5">
            {list.map((q, i) => (
              <button
                ...
                onClick={() => setIndex(i)}
                className={cn(
                  "grid aspect-square place-items-center rounded-md border border-border text-xs font-semibold transition-colors",
          ...
          <div className="mt-4 space-y-1.5 text-xs text-muted-foreground">
            <p>
              <span className="mr-2 inline-block size-3 rounded-sm bg-success align-middle" /> Answered
            </p>
            <p>
              <span className="mr-2 inline-block size-3 rounded-sm bg-warning align-middle" /> Marked for review
            </p>
          </div>
        </aside>
      </main>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You have answered {answeredCount} of {list.length} questions. Unanswered questions score zero and this
              attempt counts towards your weekly average.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
